...
### To run the **File Mode**:
The file mode, uses the sample file provided with this coding challenge (it is read from inside ./sample/input.text)
```
npm run start
```
...
### To run the **CLI Mode**:
The CLI mode, uses real time typing on the Command line Interface to enter the bouquet specs and flowers definition:
